...
case class Tile(name: String, defense: Int, moveCost: MovementCost)
...
object Tile {
  ...
  val Factory: Tile =
    Tile("Factory", 3, Map(Foot -> 1, Boots -> 1, Tires -> 1, Tread -> 1, Air -> 1))
  val Airport: Tile =
    Tile("Airport", 3, Map(Foot -> 1, Boots -> 1, Tires -> 1, Tread -> 1, Air -> 1))
  val Port: Tile = Tile(
    "Port",
    3,
